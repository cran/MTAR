...
Before setting up the `MTAR` package, users should have `R` version 3.4.0 or higher, and several packages (`MASS`, `SKAT`, `seqMeta`,`Matrix`, `CompQuadForm`) set up from CRAN.

## Installing Guide
`MTAR` package requires R with version 3.4.0 or higher, which can be downloaded and installed from [CRAN](https://cran.r-project.org/). The source `MTAR` package can be downloaded from [here](https://tangzheng1.github.io/tanglab/software.html). 

### Package dependencies

Users should install the following packages prior to installing `MTAR`, from an `R` terminal:

```
install.packages(c('SKAT', 'MASS', 'seqMeta', 'CompQuadForm', 'Matrix'))
```

which will install in about 30 seconds.

The `MTAR` package functions with all packages in their latest versions as they appear on `CRAN` on September 9, 2019. The versions of software are, specifically:
```
CompQuadForm_1.4.3
MASS_7.3-51.4
Matrix_1.2-17 
...
After downloading `MTAR` package, unzip it and you will see `MTAR_0.1.0.tar.gz`. To install `MTAR`, type the following code from an `R` session:

```
install.package("MTAR_0.1.0.tar.gz", repos = NULL)
library(MTAR)
```
